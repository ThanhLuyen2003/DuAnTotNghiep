...
const KemDanhRang = (props) => {
  const { navigation } = props;
  const [dsProductKemDanhRang, setdsProductKemDanhRang] = useState([])
  const [isLoading, setisLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const getList = async () => {


    let apiSalon = 'http://' + ip + ':3000/apiProduct/productsalon/KemDanhRang';

    try {
      const response = await fetch(apiSalon);
      const json = await response.json(); //chuyen du lieu thanh json

      setdsProductKemDanhRang(json);// do du lieu vao state
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const filteredProducts = dsProductKemDanhRang.filter((item) =>
      ...
      <View >
        {
          (isLoading)
            ...
            : <FlatList numColumns={numColumns} data={dsProductKemDanhRang} renderItem={renderProductSalon} />
